...
def increment_date(): 
    ...
    # retrieve all recent positive tests
    query = (f"""
    SELECT email FROM person WHERE medicareNumber IN (
    SELECT DISTINCT medicareNumber FROM diagnostic WHERE resultDate > (DATE '{fourteen_days_ago}') AND result=1);
        """)
    ...
    # create messages for all recent positive
    #  
    messages = ""
    index = 0
    end_character = ","
    ...
    while loop:
        ...
        messages += f" ('{data[index][email]}', 'click on recommendations tab to view current health guidlines', 'Reminder of public health recommendation'){end_character}"
        index += 1

    query = (f"""
    INSERT INTO message (email, healthRecommendation, description)
    VALUES
    {messages}
    """)
    print(query)
